...
class StuckTrackers(base.ScriptBaseWithConfig):
    ...
    def add_options(self):
        ...
        # basic options
        self.add_bool_option("-a", "--all", help="list ALL items, not just stuck ones")
        ...
        self.add_bool_option(
            "-t", "--to-tagged", help="add stuck items to 'tagged' view"
        )
    ...
    def mainloop(self):
        ...
        now = int(time.time())
        fields = (
            "is_enabled is_busy url min_interval normal_interval"
            " activity_time_last success_counter failed_counter scrape_counter"
        ).split()
        ...
        rows = proxy.d.multicall2(
            ...
            "started",
            "d.hash=",
            "t.multicall=,{}".format(",".join(["t.{}=".format(i) for i in fields])),
        )
        stuck = Counter()
        ...
        for idx, (infohash, trackers) in enumerate(rows, 1):
            ...
            if not any(t.is_enabled for t in trackers):
                ...
                stuck[domain] += 1
                print(
                    "{i:5d}  {n:>2s}  {n:>5s}  {n:>5s}  {n:>5s}  {delta:>13s}  {hash}  {domain}".format(
            ...
            for num, t in enumerate(trackers, 1):
                ...
                if self.options.all or delta > t.normal_interval:
                    ...
                    print(
                        ...
                        "{delta}  {hash}  {domain}".format(
                            t=t,
                            i=idx,
                            n=num,
                            hash=infohash,
                            domain=domain,
                            delta=fmt.human_duration(
                                t.activity_time_last, precision=2, short=True
                            ),
                        )
                    )
