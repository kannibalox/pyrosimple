...
class FilterEdit(ModalScreen):
    def compose(self):
        yield Grid(
            Input(
                value=self.app.rtorrent_filter,
                id="filter_input",
...
class TorrentInfoScreen(ModalScreen):
    BINDINGS = [("escape", "escape", "Return to main screen")]
    ...
    def load_info_data(self) -> None:
        ...
        prefetch = []
        for p in pyrosimple.torrent.rtorrent.get_fields_from_template(
            self.info_template
        ):
    ...
    def load_peer_data(self) -> None:
        ...
        for peer in result:
            ...
            table.add_row(
                data["address"],
                fmt.human_size(data["up_rate"]) + "/s"
                if int(data["up_rate"]) != 0
                else "-",
                fmt.human_size(data["down_rate"]) + "/s"
                if int(data["down_rate"]) != 0
                else "-",
                data["client_version"],
            )
...
class CollapsedTorrentTable(DataTable):
    COLUMNS = [
        (
            "❢",
            {"key": "error", "width": 1},
            "{% if d.message %}[bold red reverse]!{%endif%}",
        ),
        (
            "☢",
            {"key": "state", "width": 1},
            "{% if d.is_open and d.is_active %}▹{% else %}▪{%endif%}",
        ),
        ...
        (
            "Name",
            {"key": "name"},
            "{%set rich='magenta'%}{%if d.is_complete%}{%set rich='green'%}{%endif%}[{{rich}}]{{d.name}}",
        ),
    ]
...
class PyroUIApp(App):
    ...
    def load_engine(self) -> None:
        ...
        self.query_one(
            "#rtorrent_info"
        ).renderable = f"*** rTorrent {props['system.client_version']}/{props['system.library_version']} - {props['session.name']} ***"
    ...
    def load_data(self) -> None:
        ...
        header = self.query_one("#pyroui_header")
        is_refresh = (self.rtorrent_filter == header.filter) and (
            self.rtorrent_view == header.view
        )
        ...
        if is_refresh:
            ...
            for item in items:
                data = list(
                    pyrosimple.torrent.rtorrent.format_item(template, item).split("\t")
                )
                # Create
